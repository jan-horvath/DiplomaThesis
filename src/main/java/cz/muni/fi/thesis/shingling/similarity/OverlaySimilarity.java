...
public class OverlaySimilarity {
    ...
    public static double weighedOverlayJaccard3(OverlaySequence os1, OverlaySequence os2, int matchingsRequired) {
        ...
        for (Map.Entry<OverlaySequence.MOMW5, Double> mw1Entry : os1.getMOMWs().entrySet()) {
            int[] mw1 = mw1Entry.getKey().motionWord;
            for (Map.Entry<OverlaySequence.MOMW5, Double> mw2Entry : os2.getMOMWs().entrySet()) {
        ...
        for (Double weight : os1.getMOMWs().values()) {weightOfAllMotionWords += weight;}
        for (Double weight : os2.getMOMWs().values()) {weightOfAllMotionWords += weight;}
