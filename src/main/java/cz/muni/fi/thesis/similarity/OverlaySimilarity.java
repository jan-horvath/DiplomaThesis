...
public class OverlaySimilarity {
    ...
    public static boolean overlayMotionWordsMatch(MomwEpisode.MOMW momw1, MomwEpisode.MOMW momw2, int matchingsRequired) {
        int[] mw1 = momw1.getMW();
        int[] mw2 = momw2.getMW();
     ...
     */
//    public static double weighedOverlayJaccard3(MomwEpisode os1, MomwEpisode os2, int matchingsRequired) {
//        Set<int[]> seq1_matched = new HashSet<>();
//        Set<int[]> seq2_matched = new HashSet<>();
//
//        double weightOfMatchedMotionWords = 0;
//        for (Map.Entry<MomwEpisode.MOMW, Double> mw1Entry : os1.getMotionWords().entrySet()) {
//            int[] mw1 = mw1Entry.getKey().motionWord;
//            for (Map.Entry<MomwEpisode.MOMW, Double> mw2Entry : os2.getMotionWords().entrySet()) {
...
//    }
    ...
    private static double dtwDistance(List<MomwEpisode.MOMW> seq1, List<MomwEpisode.MOMW> seq2, int matchingsRequired) {
        int N = seq1.size();
        int M = seq2.size();
        double accumMatrix[][] = new double[N][M];
        accumMatrix[0][0] = overlayMotionWordsMatch(seq1.get(0), seq2.get(0), matchingsRequired) ? 0.0 : 1.0;
    ...
    public static double dtwSimilarity(List<MomwEpisode.MOMW> seq1, List<MomwEpisode.MOMW> seq2, int matchingsRequired) {
        return 1 - dtwDistance(seq1, seq2, matchingsRequired);
    }
}
